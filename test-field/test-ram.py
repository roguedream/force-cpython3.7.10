...
# Compiled at: 2016-06-25 15:46:10
...
def _get_real_winver(maj, min, build):
    if maj < 6 or maj == 6 and min < 2:
        ...
        while actual_len == name_len:
            ...
            actual_len = kernel32.GetModuleFileNameW(HANDLE(kernel32._handle), name, len(name))
            if not actual_len:
                return (maj, min, build)
        ...
        min = pvi.contents.dwProductVersionMS & 65535
...
def _platform(*args):
    ...
    """
    platform = string.join(list(map(string.strip, list(filter(len, args)))), '-')
