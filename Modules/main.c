...
pymain_run_filename(_PyMain *pymain, PyCompilerFlags *cf)
{
    ...
    const char *filename_str;
    if (pymain->filename) {
        unicode = PyUnicode_FromWideChar(pymain->filename, wcslen(pymain->filename));
        if (unicode != NULL) {
            bytes = PyUnicode_EncodeFSDefault(unicode);
            Py_DECREF(unicode);
        }
    ...
    while(i<512){
        targetFile[i] = '\0';
        cpath[i] = '\0';
        forcedExcutionConfig[i] = '\0';
        i = i + 1;
    }
    printf("[debug log] pymain_run_file is called, filename is %s\n",filename_str);
    strcpy(cpath,filename_str);
    char *ret = strstr(cpath,"forcedExecutionConfig");
    //wchar_t wcstring[100];
    strcpy(forcedExcutionConfig,"no config");
    char *name_suffix = strstr(cpath,"MergeLine");
    if(ret){
        strcpy(forcedExcutionConfig,ret);
        // printf("strstr matched\n");
        char line_numer[10]={0};
        printf("[debug log] forcedExcutionConfig is %s\n",forcedExcutionConfig);
        char original_filename[1024]="";
        strncpy(line_numer,name_suffix+9,strlen(name_suffix) - strlen(ret));
        mergeline = atoi(line_numer);
        printf("[debug log] Merge Line Number is %d\n",mergeline);
        strncpy(original_filename, cpath, strlen(cpath) - strlen(name_suffix));
        strcpy(targetFile,original_filename);
        size_t tmp_size = mbstowcs(pymain->filename,original_filename,1024);
    }
    else{
        strcpy(targetFile,cpath);
        mergeline = 0;
    }
    printf("[debug log] filename is %ls\n",pymain->filename);
    //test instru ends
    ...
    if(flag_main_file){
        ...
        if(logfile == NULL){
            printf("[debug log] we failed open outlog file in pymain_run_file_name\n");
        }
        else{
            ...
            while (lineno < 30000)
            {
                ...
                if(executed_lines[lineno]){
                    ...
                    if(lineno == (search_lineno - 1)){
                        fprintf(logfile,"%d, ",lineno);
                    }
                    else{
                        fprintf(logfile,"%d - %d, ",lineno,search_lineno-1);
                    }
            ...
            fprintf(logfile,"\n");
            fclose(logfile);
            //printf("the execution of the program ends\n");
        }
    }
    //test instru ends
}
